# Derived version of https://github.com/huacnlee/activestorage-aliyun
...
module ActiveStorage
  class Service::AliyunService < Service
    ...
    @@max_characters_for_subfolder = 10
    ##
    # Instead of flat full string of @key as location, this breaks up key into 
    # subfolders by @@max_characters_for_subfolder characters.
    def path_for(key)
      root_path = config.fetch(:path, nil)
      key_copy = key.clone.gsub(/\A(variants\/|\/)/, '')
      subpath = key.starts_with?('variants/') ? 'variants/' : ''
      0.upto(key_copy.length - 1) do|index|
        subpath << key_copy[index]
        subpath << '/' if index > 0 && index % @@max_characters_for_subfolder == 0
      end
      if root_path.blank? || root_path == ''
        full_path = subpath
      else
        full_path = File.join(root_path, subpath)
      end
      full_path.gsub(/^\//, '')
    end
    ...
    def endpoint
      config.fetch(:endpoint, DEFAULT_ENDPOINT)
    end

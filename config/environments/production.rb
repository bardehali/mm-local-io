require File.join( Rails.root, 'app/mailers/dynamic_smtp_settings_interceptor.rb' )
...
Rails.application.configure do
  ...
  #Configures to use the X-Forwarded-For header to get the real IP address.
  #Specifically for puma server running on the load balancer's machine which is located at 93.123.72.31
  config.action_dispatch.trusted_proxies = %w(93.123.72.31).map { |proxy| IPAddr.new(proxy) }

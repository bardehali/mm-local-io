...
//= require spree/frontend/views/spree/shared/variant_select
...
function CartForm($, $cartForm) {
  ...
  this.updateVariantId = function() {
    ...
    if (typeof(getVariantData) == 'function') { getVariantData(); }
  }
...
$('#product-details').on('submit', ADD_TO_CART_FORM_SELECTOR, function(event) {
    if (SpreeAPI.oauthToken != null) { return; }
    ...
    if ($cartForm.attr('data-remote') == 'true') // remote form
    {
    } 
    else {

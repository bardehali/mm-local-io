...
(function ($) { 
	...
	function toggleRatingStars(){ 
		$(this).parent().removeClass('star-rating-off').addClass('star-rating-on');
		var starNo = parseInt( $(this).val() );
		$(this).parent().siblings().each(function(index){
			if (index < starNo - 1) {
				$(this).removeClass('star-rating-off').addClass('star-rating-on');
			} else {
				$(this).removeClass('star-rating-on').addClass('star-rating-off');
			}
		});
	}
	$(".star-rating input").on('change', toggleRatingStars);
	$(".star-rating input").on('click', toggleRatingStars);
})(jQuery);
